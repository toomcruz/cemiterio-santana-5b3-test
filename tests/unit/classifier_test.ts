...
Deno.test("classifier uses pending state before interpreting Sim", async () => {
  ...
  const output = await classifyDeterministically({
    ...
    message_batch: batchFixture("sim"),
...
Deno.test("classifier blocks automatic handling while human is active", async () => {
  const output = await classifyDeterministically({
    ...
    correlation_id: ids.decision,
    release_id: ids.release,
    state: stateFixture({ automation_mode: "HUMAN_ACTIVE", human_handoff_active: true }),
    taxonomy: taxonomyFixture(),
    message_batch: batchFixture("quero recadastro"),
    technical_signals: { duplicate: false, human_active: true, provider_window_open: true, inbound_source: "WAPI" },
  });
...
Deno.test("classifier recognizes internal complaint intent without creating a request", async () => {
  const output = await classifyDeterministically({
    ...
    correlation_id: ids.decision,
    release_id: ids.release,
    state: stateFixture({ active_topic: null }),
    taxonomy: taxonomyFixture(),
    message_batch: batchFixture("Quero fazer uma reclamação sobre o atendimento"),
    technical_signals: { duplicate: false, human_active: false, provider_window_open: true, inbound_source: "WAPI" },
  });
...
Deno.test("ambiguous batch returns no response text and no invented classification", async () => {
  const output = await classifyDeterministically({
    ...
    state: stateFixture({ active_topic: null }),
    taxonomy: taxonomyFixture(),
    message_batch: batchFixture("olá"),
    technical_signals: { duplicate: false, human_active: false, provider_window_open: true, inbound_source: "WAPI" },
  });
...
Deno.test("aggregated batch drives topic change without discarding the old topic", async () => {
  const output = await classifyDeterministically({
    ...
    state: stateFixture(),
    taxonomy: taxonomyFixture(),
    message_batch: batchFixture("agora preciso de exumação", {
      message_ids: [ids.message, "12121212-1212-4121-8121-121212121212"],
      quiet_seconds: 7,
    }),
    technical_signals: { duplicate: false, human_active: false, provider_window_open: true, inbound_source: "WAPI" },
  });
