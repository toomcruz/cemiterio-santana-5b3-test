...
Deno.test("PROPOSE sends only decision id to authoritative RPC", async () => {
  const r: any = await executeRequestCommand({
    action: "PROPOSE",
    correlation_id: "c",
    decision_id: "d",
    shadow_only: true,
  }, rest);
  assertEquals(r.reason_codes[0], "propose_request_transaction");
});
Deno.test("CONFIRM requires inbound proof", async () => {
  let failed = false;
  try {
