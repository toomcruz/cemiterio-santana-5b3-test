 ...
 */
async function expectAConfirmarReject(label: string, operation: () => Promise<unknown>, expected: "A_CONFIRMAR_NO_FACTS" | "DECISION_CANNOT_PROPOSE"): Promise<void> {
  try {
    await operation();
  } catch (error) {
    ...
    if (expected === "DECISION_CANNOT_PROPOSE" && (error.code !== "SUPABASE_REST_ERROR" || !error.message.toLowerCase().includes("decision cannot propose"))) {
...
Deno.test({ name: "P10 integration: database rules -> engine -> renderer/proposal", ignore: true }, async () => {
  ...
  const plan = await decide({
    ...
    state: stateFixture(),
    classification: {
      schema_version: "1.0", correlation_id: ids.decision, release_id: releaseId,
      classification_status: "OK", message_role: "CONTINUATION", intent_candidates: [],
      service_candidates: [], location_candidate: null, complaint_signal: false,
      human_need_signal: false, topic_transition_candidate: "KEEP_ACTIVE",
      continuation_of_topic_id: topicId, document_signal: "NONE", ambiguity_codes: [], evidence: [],
    },
    technical_context: { now: new Date().toISOString(), provider_window_open: true, channel: "WHATSAPP", human_active: false, duplicate: false },
  }, { rpc: async (name: string) => name === "get_runtime_decision_rules" ? rules : (() => { throw new Error(`unexpected RPC ${name}`); })() } as never);
  ...
  await expectAConfirmarReject("renderer", () => render({ decision_id: decisionId, technical: { channel: "WHATSAPP" } }, rest), "A_CONFIRMAR_NO_FACTS");
  await expectAConfirmarReject("proposal", () => rest.rpc("propose_request_transaction", { p_decision_id: decisionId, p_actor: "P10" }), "DECISION_CANNOT_PROPOSE");
  ...
  const requests = await fetch(`${base}service_requests?session_id=eq.${sessionId}&select=id,protocol`, { headers }).then((r) => r.json());
