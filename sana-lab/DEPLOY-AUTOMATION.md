...
- Rollback sintético: `workflow_dispatch` com `commit_sha` = HEAD da branch e `mode=rollback-test`, **quando este workflow também estiver presente na branch padrão do repositório**. GitHub exige que workflows acionados manualmente existam na branch padrão; o gatilho `push` nesta branch funciona sem essa condição. Não instalar o workflow no `main` sem revisão explícita de seu escopo. Uma vez disponível, o job deve mostrar `LAB_PROBE_OK`, `SYNTHETIC_LAB_FAILURE_AFTER_HEALTH_PROBE` e `ROLLBACK_CONFIRMED=YES`, sem `DEPLOY_OK`.
