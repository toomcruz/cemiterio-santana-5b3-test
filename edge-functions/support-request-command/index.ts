...
function one<T>(value: T | T[]): T {
  if (Array.isArray(value)) {
    if (!value[0]) throw new HttpProblem(502, "EMPTY_RPC_RESULT", "RPC returned no result");
    return value[0];
  }
  return value;
}
...
export async function executeRequestCommand(
  ...
  if (input.action === "PROPOSE") {
    return one(
      await rest.rpc<RequestCommandResult | RequestCommandResult[]>("propose_request_transaction", {
  ...
  if (input.action === "CONFIRM") {
    return one(
      await rest.rpc<RequestCommandResult | RequestCommandResult[]>("confirm_request_transaction", {
  ...
  if (input.action === "DECLINE") {
    return one(
      await rest.rpc<RequestCommandResult | RequestCommandResult[]>("decline_request_transaction_v2", {
        p_confirmation_id: id,
        p_confirmation_nonce: assertString(input.confirmation_nonce, "confirmation_nonce"),
        p_actor: actor,
      }),
  ...
  if (input.action === "GET_STATUS") {
    return one(
      await rest.rpc<RequestCommandResult | RequestCommandResult[]>("get_request_confirmation_status", {
        p_confirmation_id: id,
      }),
    );
  }
...
if (import.meta.main) {
  Deno.serve(async (request) => {
    try {
      ...
      requireShadowOnly();
      return json(await executeRequestCommand(await parseJson<RequestCommandInput>(request), new SupabaseRest()));
    } catch (error) {
