...
function evidence(kind: ClassifierEvidence["kind"], summary: string, input: ClassifierInput): ClassifierEvidence {
  return { evidence_id: newId(), kind, message_id: input.message_batch.message_ids.at(-1), redacted_summary: summary.slice(0, 240) };
}
...
function pickService(text: string, input: ClassifierInput, evidences: ClassifierEvidence[]) {
  ...
  for (const service of input.taxonomy.services) {
    ...
    const alias = service.aliases.find((candidate) => candidate.trim().length >= 2 && new RegExp(`(^|\\s)${escapeRegExp(candidate.trim())}(?=\\s|$|[,.!?])`, "i").test(text));
    if (alias) {
      const itemEvidence = evidence("TEXT_SPAN", `Alias publicado correspondente a ${service.code}`, input);
      evidences.push(itemEvidence);
      candidates.push({ code: service.code, confidence: 0.9, source: "DETERMINISTIC", evidence_ids: [itemEvidence.evidence_id] });
    }
  }
...
export async function classifyDeterministically(input: ClassifierInput): Promise<ClassifierOutput> {
  ...
  if (input.technical_signals.human_active || input.state.automation_mode === "HUMAN_ACTIVE") return blocked(input, "HUMAN_ACTIVE", evidences);
  ...
  if (pending && pending.status === "PENDING" && new Date(pending.expires_at) > new Date(input.message_batch.received_at) && input.state.active_topic?.topic_id === pending.topic_id && AFFIRMATIVE.test(text)) {
  ...
  } else if (pending && pending.status === "PENDING" && new Date(pending.expires_at) > new Date(input.message_batch.received_at) && input.state.active_topic?.topic_id === pending.topic_id && NEGATIVE.test(text)) {
    const itemEvidence = evidence("PENDING_QUESTION", "Confirmação pendente válida no tópico ativo", input);
    evidences.push(itemEvidence);
  ...
  } else if (question && question.status === "OPEN" && (!question.expires_at || new Date(question.expires_at)>new Date(input.message_batch.received_at)) && /^\d+\s*$/.test(text) && numericAnswerAllowed(text,question.expected_answer_schema)) {
    const itemEvidence = evidence("PENDING_QUESTION", `Resposta numérica vinculada à pergunta ${question.question_code}`, input);
    evidences.push(itemEvidence);
  ...
  const intentCandidates = complaintSignal
    ? (() => {
      ...
      evidences.push(itemEvidence);
      return [{ code: "RECLAMACAO_INTERNA", confidence: 0.95, source: "DETERMINISTIC" as const, evidence_ids: [itemEvidence.evidence_id] }];
    })()
    : [];
  ...
  if (input.state.active_topic && serviceCandidates[0] && input.state.active_topic.service_code && serviceCandidates[0].code !== input.state.active_topic.service_code) {
    transition = "START_NEW";
    message_role = "TOPIC_CHANGE";
  }
  if (message_role === "UNKNOWN" || (message_role === "NEW_TOPIC" && !complaintSignal && serviceCandidates.length === 0)) ambiguityCodes = ["NO_CONFIDENT_SERVICE_OR_INTENT"];

  return {
    schema_version: "1.0", correlation_id: input.correlation_id, release_id: input.release_id,
    classification_status: ambiguityCodes.length ? "AMBIGUOUS" : "OK", message_role,
    intent_candidates: intentCandidates, service_candidates: serviceCandidates,
    location_candidate: locationCandidate, complaint_signal: complaintSignal,
    human_need_signal: HUMAN.test(text), topic_transition_candidate: ambiguityCodes.length ? "AMBIGUOUS" : transition,
    continuation_of_topic_id: message_role === "CONTINUATION" || message_role === "ANSWER_TO_PENDING_QUESTION" ? input.state.active_topic?.topic_id ?? null : null,
    document_signal: input.message_batch.attachments.length ? "TECHNICAL_DOCUMENT" : "NONE",
    ambiguity_codes: ambiguityCodes, evidence: evidences,
  };
}
...
function blocked(input: ClassifierInput, code: string, evidenceList: ClassifierEvidence[]): ClassifierOutput {
  ...
  return {
    schema_version: "1.0", correlation_id: input.correlation_id, release_id: input.release_id,
    classification_status: "BLOCKED", message_role: "UNKNOWN", intent_candidates: [], service_candidates: [], location_candidate: null,
    complaint_signal: false, human_need_signal: code === "HUMAN_ACTIVE", topic_transition_candidate: "KEEP_ACTIVE",
    continuation_of_topic_id: input.state.active_topic?.topic_id ?? null, document_signal: "NONE", ambiguity_codes: [code], evidence: evidenceList,
  };
}
...
function numericAnswerAllowed(text:string,schema:Record<string,unknown>):boolean {
  const n=Number(text); if(!Number.isSafeInteger(n)) return false;
  const min=typeof schema.minimum==='number'?schema.minimum:undefined; const max=typeof schema.maximum==='number'?schema.maximum:undefined;
...
if (import.meta.main) Deno.serve(async (request) => {
  try {
    ...
    const persistedInbound = await rest.rpc<{ content_hash: string } | Array<{ content_hash: string }>>("persist_shadow_inbound_message", {
    ...
    const inboundContentHash = Array.isArray(persistedInbound) ? persistedInbound[0]?.content_hash : persistedInbound?.content_hash;
    if (!inboundContentHash) throw new HttpProblem(502, "INBOUND_HASH_MISSING", "Inbound persistence did not return an authoritative content hash");
    ...
    if (errors.length) output = { ...output, classification_status: "INVALID_INPUT", ambiguity_codes: [...output.ambiguity_codes, ...errors] };
    if (input.state.active_topic) {
      const classificationCode = output.message_role === "CONFIRMATION_AFFIRMATIVE" ? "CONFIRMATION_AFFIRMATIVE" : "OTHER";
      const classificationStatus = output.classification_status === "OK" ? "OK" : output.classification_status === "BLOCKED" ? "BLOCKED" : "AMBIGUOUS";
      const confirmationId = classificationCode === "CONFIRMATION_AFFIRMATIVE" ? input.state.pending_confirmation?.confirmation_id ?? null : null;
      const authorityNonce = classificationCode === "CONFIRMATION_AFFIRMATIVE" ? newId() : null;
      const authorityKeyId = classificationCode === "CONFIRMATION_AFFIRMATIVE" ? Deno.env.get("SUPPORT_CLASSIFIER_AUTHORITY_KEY_ID") ?? null : null;
      const authoritySecret = classificationCode === "CONFIRMATION_AFFIRMATIVE" ? Deno.env.get("SUPPORT_CLASSIFIER_ASSERTION_KEY") ?? null : null;
      if (classificationCode === "CONFIRMATION_AFFIRMATIVE" && (!authorityKeyId || !authoritySecret || !confirmationId || !authorityNonce)) {
      ...
      const assertionMaterial = classificationCode === "CONFIRMATION_AFFIRMATIVE"
        ? await rest.rpc<string>("classifier_assertion_material", { p_inbound_message_id: inboundMessageId, p_content_hash: inboundContentHash, p_confirmation_id: confirmationId, p_session_id: input.state.session_id, p_topic_id: input.state.active_topic.topic_id, p_release_id: input.release_id, p_classification_code: classificationCode, p_classification_status: classificationStatus, p_classifier_version: CLASSIFIER_VERSION, p_authority_nonce: authorityNonce })
        : null;
      const authorityAssertion = assertionMaterial && authoritySecret ? await hmacSha256(authoritySecret, assertionMaterial) : null;
    ...
    await appendAuditEvent(rest, {
      correlation_id: input.correlation_id, component: "support-classifier", component_version: "5B.1",
      event_type: "CLASSIFICATION_RUN", outcome: output.classification_status, actor_type: "EDGE_FUNCTION",
      conversation_id: input.state.conversation_id, session_id: input.state.session_id, topic_id: input.state.active_topic?.topic_id,
      release_id: input.release_id,
      metadata_redacted: { message_role: output.message_role, intent_codes: output.intent_candidates.map((item) => item.code), service_codes: output.service_candidates.map((item) => item.code), ambiguity_codes: output.ambiguity_codes, complaint_signal: output.complaint_signal },
    });
    return json(output);
  } catch (error) {
