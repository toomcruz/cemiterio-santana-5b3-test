...
export async function render(input: RendererInput, rest: SupabaseRest) {
  ...
  if (!p || c.outcome === "A_CONFIRMAR") {
    throw new HttpProblem(409, "A_CONFIRMAR_NO_FACTS", "No factual rendering allowed");
  }
  if (p.mode === "GEMINI") throw new HttpProblem(503, "GEMINI_DISABLED", "Gemini needs specific approval");
  return {
    decision_id: c.decision_id,
    mode: p.mode,
    template_id: p.template_id ?? null,
    authorized_field_keys: Object.keys(p.template_variables ?? {}),
    assets: p.asset_ids ?? [],
    shadow_only: true,
  };
}
if (import.meta.main) {
  Deno.serve(async (request) => {
    try {
      ...
      requireShadowOnly();
      return json(await render(await parseJson<RendererInput>(request), new SupabaseRest()));
    } catch (error) {
