...
} from "../../contracts/confirmation-authorization.ts";
...
export async function authorizeConfirmation(
  input: ConfirmationAuthorizationInput,
  rest: SupabaseRest,
): Promise<ConfirmationAuthorizationResult> {
...
if (import.meta.main) {
  Deno.serve(async (request) => {
    try {
      ...
      requireShadowOnly();
      return json(
        await authorizeConfirmation(await parseJson<ConfirmationAuthorizationInput>(request), new SupabaseRest()),
      );
    } catch (error) {
