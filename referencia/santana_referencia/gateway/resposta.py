...
class RespostaAutoritativa:
    ...
    # Opcoes por campo. Substitui a antiga `opcoes_possiveis` plana: com mais de
    # um campo faltante a lista plana nao dizia a qual campo cada opcao
    # pertencia. Chaves e valores ordenados por code point.
    opcoes_por_campo: Mapping[str, tuple[str, ...]] = field(default_factory=dict)
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        if self.opcoes_por_campo:
            dados["opcoes_por_campo"] = {
                chave: list(self.opcoes_por_campo[chave])
                for chave in sorted(self.opcoes_por_campo)
            }
        return dados
